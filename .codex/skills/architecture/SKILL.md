...
The current backend-backed workspace identity is loaded through `src/services/workspaceService.js`. It returns the user’s organization memberships, marks active and suspended access explicitly, and selects an active organization when one is available; the selected ID is stored through the shared active-organization service helper. Domain pages should follow the same service boundary and must not render local fixture records.
